...
window.onload = function () {
    ...
    var tags = ["Pin EVE", "Pin Makita", "Pin Dewalt", "Mạch BMS", "BMS JK"];
...
function phanTich_URL_chiTietSanPham() {
    ...
    var s = addThongSo('Dung lượng', sanPhamHienTai.detail.Capacity);
    s += addThongSo('Tuổi thọ', sanPhamHienTai.detail.Cycle);
    s += addThongSo('Dòng xả tối đa', sanPhamHienTai.detail.MaxCharge);
    s += addThongSo('Dòng sạc tối đa', sanPhamHienTai.detail.MaxDisCharge);
    s += addThongSo('Màu sắc', sanPhamHienTai.detail.Color);
    s += addThongSo('Kích thước', sanPhamHienTai.detail.Size);
	s += addThongSo('Tình trạng', sanPhamHienTai.detail.Status);
	s += addThongSo('Vật liệu', sanPhamHienTai.detail.Material);
    info.innerHTML = s;
